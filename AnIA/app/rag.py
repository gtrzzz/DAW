...
async def index_knowledge() -> dict[str, int]:
    ...
    for file_path in files:
        ...
        topic = relative_path.parts[0] if len(relative_path.parts) > 1 else "general"
        ...
        for index, chunk in enumerate(chunk_text(text)):
            ...
            collection.upsert(
                ...
                metadatas=[{"path": relative_path.as_posix(), "topic": topic, "chunk": index}],
            )
            indexed_chunks += 1
...
async def retrieve_context(question: str, top_k: int | None = None) -> tuple[str, list[RagSource]]:
    ...
    for document, metadata in zip(documents, metadatas, strict=False):
        ...
        context_parts.append(f"Fuente: {source.path}\n{document}")
